...
$(document).on('click', '.ViewBageOldPath', function () {
    ...
    if (folderN == "D:\\Zippy-CRM\\ZippyCRM\\File\\DocumentManager\\Customer\\" + customerId || folderN=="")
    {
...
$(document).on('click', '#renameButton', function () {
    ...
    debugger;
    if (folderNew) { // Check if the new name is not empty
...
function RenameFolder(folderN, folderCurrent, folderNew) {
    ...
    $.ajax({
        ...
        type: 'POST',
        data: { folderN: folderN, currentFolder: folderCurrent, newFolderName: folderNew },
        success: function (result) {
